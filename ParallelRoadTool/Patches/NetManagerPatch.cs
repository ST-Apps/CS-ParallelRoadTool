...
[HarmonyPatch(typeof(NetManager), nameof(NetManager.CreateSegment),
              new[]
              {
                  typeof(ushort), typeof(Randomizer), typeof(NetInfo), typeof(TreeInfo), typeof(ushort), typeof(ushort), typeof(Vector3),
                  typeof(Vector3), typeof(uint), typeof(uint), typeof(bool)
              },
              new[]
              {
                  ArgumentType.Out, ArgumentType.Ref, ArgumentType.Normal, ArgumentType.Normal, ArgumentType.Normal, ArgumentType.Normal,
                                ...
                                IList<object>  __args)
    {
        try
        {
            ...
            if (Singleton<ParallelRoadToolManager>.instance.IsMouseLongPress           &&
                ToolsModifierControl.GetTool<NetTool>().m_mode == NetTool.Mode.Upgrade && startNode == _startNodeId[0] && endNode == _endNodeId[0])
            {
            ...
            for (var i = 0; i < Singleton<ParallelRoadToolManager>.instance.SelectedNetworkTypes.Count; i++)
            {
                ...
                switch (invert)
                {
                    case false when _endNodeId[i].HasValue && _endNodeId[i].Value == startNode:
                        ...
                        if (ToolsModifierControl.GetTool<NetTool>().m_mode == NetTool.Mode.Straight && previousEndPointNullable.HasValue &&
                            previousStartPointNullable.HasValue)
                        {
                            ...
                            var intersection = NodeUtils.FindIntersectionByOffset(newStartNode.m_position, endNetNode.m_position, endDirection,
                                                                                  previousPoint.m_position, -PreviousEndDirection(i),
                ...
                if (isReversed)
                {
                    ...
                    __result = NetManagerReversePatch.CreateSegment(NetManager.instance, out segment, ref randomizer, selectedNetInfo, treeInfo,
                                                                    newEndNodeId, newStartNodeId, tempStartDirection, tempEndDirection,
                ...
                else
                {
                    ...
                    __result = NetManagerReversePatch.CreateSegment(NetManager.instance, out segment, ref randomizer, selectedNetInfo, treeInfo,
                                                                    newStartNodeId, newEndNodeId, startDirection, endDirection,
                                                                    Singleton<SimulationManager>.instance.m_currentBuildIndex + 1,
                                                                    Singleton<SimulationManager>.instance.m_currentBuildIndex, invert);
                }
