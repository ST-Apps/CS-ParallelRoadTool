...
internal class NetToolNodePatch
{
                                ...
                                out Dictionary<int, NetTool.ControlPoint[]> __state)
    {
        ...
        try
        {
            ...
            foreach (var o in ActionsManager.Execute())
            {
                if (o == null) continue;
                switch (o.ActionType)
                {
                    ...
                    case ActionFactory.ActionType.CreateSegment:
                        if (o is not CreateSegmentActionResult createSegmentActionResult)
                        {
                            Log.Error($"[{nameof(NetToolNodePatch)}.{nameof(Prefix)}] Error while processing actions: {nameof(createSegmentActionResult)} expected to be a {nameof(CreateSegmentActionResult)} but it was a {o.GetType()} instead.");
                            break;
                        }

                        __state[createSegmentActionResult.NetworkIndex] = new[]
                        {
                            createSegmentActionResult.TargetStartPoint, createSegmentActionResult.TargetEndPoint
                        };
                        break;
