...
namespace ParallelRoadTool.Detours
{
    ...
    public struct NetManagerDetour
    {
        ...
        private static Vector3 Offset(Vector3 point, Vector3 direction, float horizontalDistance,
            float verticalDistance, bool isClockwise = true, bool isLeftHand = false)
        {
            var offsetPoint = point + horizontalDistance * new Vector3((isClockwise || isLeftHand ? 1 : -1) * direction.z,
                                  direction.y,
                                  (isClockwise || isLeftHand ? -1 : 1) * direction.x);
