...
internal static class NodeUtils
{
    ...
    public static ushort NodeAtPositionOrNew(ref Randomizer randomizer, NetInfo info, Vector3 newNodePosition, float verticalOffset)
    {
        ...
        // var newNodeId = NodeIdAtPosition(newNodePosition, info);
    ...
    /// <returns></returns>
    public static ref NetNode FromPosition(Vector3 position)
    {
        return ref FromId(NodeIdAtPosition(position));
    }

    /// <summary>
    ///     Searches for a node with the provided <see cref="NetInfo" /> (if any) at the given <see cref="Vector3" /> position.
    /// </summary>
    /// <param name="position"></param>
    /// <param name="info"></param>
    /// <returns></returns>
    ...
    /// <summary>
    ///     Defines a range of cells to look for a node.
    /// </summary>
    /// <param name="position">Position for the <see cref="NetNode" /> we're looking for.</param>
    /// <param name="rangeRow">
    ///     Pair with <see cref="FixedVector2D.x" /> as minimum index and <see cref="FixedVector2D.y" /> as
    ///     maximum one.
    /// </param>
    /// <param name="rangeColumn">
    ///     Pair with <see cref="FixedVector2D.x" /> as minimum index and <see cref="FixedVector2D.y" />
    ///     as maximum one.
    /// </param>
